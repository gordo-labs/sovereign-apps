...
export function validateConfig(input) {
  ...
  if (!SAFE.test(config.namespace)) errors.push('namespace must be lowercase kebab-case (1-63 characters)');
  ...
  for (const id of config.modules) {
    ...
    if (!spec) continue;
    const missing = (spec.requires ?? []).filter((dependency) => !config.modules.includes(dependency));
    ...
    if (!supported && !spec.optional) errors.push(`module ${id} has no adapter for selected platforms (${config.platforms.join(', ')})`);
  }
  if (config.modules.includes('electron') && !config.platforms.includes('electron')) errors.push('module electron requires platform electron');
  if (config.modules.includes('reactNative') && !config.platforms.some((platform) => ['android', 'ios'].includes(platform))) errors.push('module reactNative requires android or ios');
  if (config.modules.includes('qr') && (!config.platforms.includes('electron') || !config.platforms.some((platform) => ['android', 'ios'].includes(platform)))) errors.push('module qr requires both an Electron endpoint and an Android/iOS client');
  if (config.modules.includes('nextExample') && !config.modules.includes('webPresence')) errors.push('module nextExample requires module webPresence');
  const output = path.resolve(config.output);
  if (config.output.split(/[\\/]/).includes('..')) errors.push('output may not contain parent-directory traversal');
  if (path.basename(output) !== config.name && config.output !== '.') errors.push(`output basename must match name (${config.name})`);
...
function manifest(config) {
  return {
    ...
    modules: config.modules.map((id) => ({ id, kind: MODULES[id].kind, optional: MODULES[id].optional === true })),
    boundaries: { generated: ['sovereign-app.json', 'src/', 'README.md', 'package.json'], adopterOwned: ['app/', 'config/local/', 'secrets/'] },
...
function filesFor(config) {
  ...
  files.set('package.json', json({ name: config.name, private: true, version: '0.1.0', type: 'module', scripts: { check: 'node src/check.mjs' }, license: 'MIT' }));
...
export async function generate(configInput, { dryRun = false } = {}) {
  ...
  const root = path.resolve(rendered.config.output === '.' ? path.join(process.cwd(), rendered.config.name) : rendered.config.output);
  if (dryRun) return { ...rendered, root };
  try {
    const existing = await fs.readdir(root);
    if (existing.length && !existing.includes('sovereign-app.json')) throw new Error(`refusing to write non-generated directory: ${root}`);
  } catch (error) {
