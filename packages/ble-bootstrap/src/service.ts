...
export interface BleBootstrapOptions extends FragmentOptions {
  readonly timeoutMs?: number;
  readonly now?: () => number;
}
export interface BleBootstrapTransport {
  readonly availability: BleAvailability;
  transfer(
    advertisement: BleAdvertisement,
    payload: Uint8Array,
    options?: BleBootstrapOptions,
  ): Promise<Uint8Array>;
}
...
export class BleBootstrapSession implements BleBootstrapTransport {
  ...
  async getAvailability(): Promise<BleAvailability> {
    return this.adapter.availability();
  }
  get availability(): BleAvailability {
    return {
      available: false,
      code: 'unknown',
      platform: this.adapter.platform,
      role: this.adapter.role,
      detail: 'Call getAvailability() before use.',
    };
  }
  async transfer(
    advertisement: BleAdvertisement,
    ...
    if (!availability.available)
      throw new Error(`BLE unavailable (${availability.code}): ${availability.detail}`);
...
export async function receiveBootstrap(
  connection: BleConnection,
  options: { timeoutMs?: number; now?: () => number; signal?: AbortSignal } = {},
): Promise<Uint8Array> {
  ...
  return new Promise<Uint8Array>((resolve, reject) => {
    ...
    const abort = () => {
      session.cancel();
      finish(() => reject(new Error('BLE bootstrap cancelled')));
    };
    options.signal?.addEventListener('abort', abort, { once: true });
    timer = setTimeout(() => {
      session.cancel();
      finish(() => reject(new Error('BLE bootstrap timed out')));
    }, timeoutMs);
  });
}
