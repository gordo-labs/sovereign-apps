...
export type BleAvailabilityCode =
  | 'available'
  | 'adapter_missing'
  | 'permission_denied'
  | 'bluetooth_off'
...
export interface BleConnection {
  ...
  onNotification(listener: (value: Uint8Array) => void): () => void;
  close(): Promise<void>;
}
...
class FakeConnection implements BleConnection {
  ...
  notify(value: Uint8Array): void {
    for (const listener of this.listeners) listener(new Uint8Array(value));
  }
...
export function createFakeBlePair(options: { mtu?: number } = {}): {
  central: BleAdapter;
  peripheral: BleAdapter;
  advertisement: BleAdvertisement;
} {
  const mtu = options.mtu ?? 185;
  const centralConnection = new FakeConnection('fake-peripheral', mtu, (value) =>
    peripheralConnection.notify(value),
  );
  const peripheralConnection = new FakeConnection('fake-central', mtu, (value) =>
    centralConnection.notify(value),
  );
  const availability = (platform: BlePlatform, role: BleRole): BleAvailability => ({
    available: true,
    code: 'available',
    platform,
    role,
    detail: 'In-memory test adapter',
  });
  const advertisement: BleAdvertisement = {
    ephemeralSessionId: 'fake-session',
    serviceUuid: '6f766572-7365-7265-6967-6e2d626c65',
    deviceHandle: 'fake-peripheral',
  };
  const central: BleAdapter = {
    platform: 'unknown',
    role: 'central',
    availability: async () => availability('unknown', 'central'),
    requestPermission: async () => availability('unknown', 'central'),
    scan: async function* () {
      yield advertisement;
    },
    connect: async (candidate) => {
      if (candidate.deviceHandle !== advertisement.deviceHandle)
        throw new Error('Unknown fake device');
  ...
  const peripheral: BleAdapter = {
    platform: 'unknown',
    role: 'peripheral',
    availability: async () => availability('unknown', 'peripheral'),
    requestPermission: async () => availability('unknown', 'peripheral'),
    scan: async function* () {},
    connect: async () => peripheralConnection,
  };
  return { central, peripheral, advertisement };
}

export function unavailableAvailability(
  platform: BlePlatform,
  role: BleRole,
  code: Exclude<BleAvailabilityCode, 'available'> = 'adapter_missing',
): BleAvailability {
  const detail =
    code === 'adapter_missing'
      ? 'No platform BLE adapter is bundled; install and inject one for this role.'
      : `BLE is unavailable: ${code}`;
...
export function createUnavailableAdapter(platform: BlePlatform, role: BleRole): BleAdapter {
  return {
    ...
    scan: async function* () {
      throw new Error('BLE adapter is not installed');
    },
    connect: async () => {
      throw new Error('BLE adapter is not installed');
    },
