...
test('fragments and reassembles out of order with duplicate', async () => {
  ...
  const fragments = await fragmentBootstrap(input, {
    mtu: 23,
    transferId: new Uint8Array([1, 2, 3, 4, 5, 6, 7, 8]),
  });
...
test('rejects tampering, conflicting duplicate and incomplete data', async () => {
  const fragments = await fragmentBootstrap(new Uint8Array(100), {
    mtu: 23,
    transferId: new Uint8Array([1, 2, 3, 4, 5, 6, 7, 8]),
  });
  const tampered = new Uint8Array(fragments[0].wire);
  tampered[tampered.length - 1] ^= 1;
  await assert.rejects(
    () => reassembleBootstrap([tampered, ...fragments.slice(1)]),
    /integrity|duplicate|different|incomplete/,
  );
  await assert.rejects(() => reassembleBootstrap(fragments.slice(0, -1)), /incomplete/);
  const conflicting = new Uint8Array(fragments[0].wire);
  conflicting[conflicting.length - 1] ^= 1;
  await assert.rejects(
    () => reassembleBootstrap([fragments[0], conflicting, ...fragments.slice(1)]),
...
test('enforces size, sequence and timeout/cancellation', async () => {
  ...
  const fragments = await fragmentBootstrap(new Uint8Array([1, 2]), {
    transferId: new Uint8Array([1, 2, 3, 4, 5, 6, 7, 8]),
  });
  const bad = new Uint8Array(fragments[0].wire);
  bad[14] = 1;
  assert.throws(() => decodeFragment(bad), /sequence/);
  let now = 0;
  const session = new ReassemblySession(10, () => now);
  session.push(fragments[0]);
  now = 11;
  assert.throws(() => session.push(fragments[0]), /timed out/);
  const cancelled = new ReassemblySession();
  cancelled.cancel();
  assert.throws(() => cancelled.push(fragments[0]), /cancelled/);
});
...
test('fake central/peripheral transfers bootstrap and cleans up', async () => {
  ...
  assert.deepEqual(
    await sender.transfer(advertisement, payload, {
      mtu: 23,
      transferId: new Uint8Array([8, 7, 6, 5, 4, 3, 2, 1]),
    }),
    payload,
  );
