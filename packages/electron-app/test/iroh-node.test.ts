...
function fakeNative(nodeId: string, relayUrl: string | null = 'https://relay.example') {
  ...
  return {
    ...
    discoveryInfo: async () => ({ nodeId, directAddress: '192.168.1.10:42424', directAddresses: ['192.168.1.10:42424'], relayUrl }),
    incoming: () => (async function* () { await new Promise<void>(() => undefined); })(),
    dial: async () => { throw new Error('fake dial'); },
...
test('uses real-node factory and classifies direct and relay candidates', async () => {
  ...
  await assert.rejects(() => node.dial('peer-b'), (error: unknown) => error instanceof IrohRuntimeError && error.state === 'closed');
});

test('missing native runtime is an observable hard error', async () => {
  await assert.rejects(() => ElectronIrohNode.create({ key: new Uint8Array(32).fill(2), nativeFactory: async () => { throw new Error('addon missing'); } }), (error: unknown) => error instanceof IrohRuntimeError && error.state === 'unavailable');
});

test('real local two-node test is opt-in and capability gated', { skip: process.env.RUN_REAL_IROH_TESTS !== '1' }, async () => {
