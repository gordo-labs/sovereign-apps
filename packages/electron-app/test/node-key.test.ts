...
test('persists independent Iroh and app identities', () => {
  ...
  assert.equal(
    Buffer.from(first.iroh.bytes).toString('hex'),
    Buffer.from(second.iroh.bytes).toString('hex'),
  );
  assert.equal(
    Buffer.from(first.app.bytes).toString('hex'),
    Buffer.from(second.app.bytes).toString('hex'),
  );
...
test('does not silently replace a corrupted identity', () => {
  ...
  assert.throws(
    () => loadOrCreateIdentities({ dataDir }),
    (error: unknown) => error instanceof CorruptIdentityError,
  );
