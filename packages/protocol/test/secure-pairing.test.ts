...
test('pairing input rejects tampering, downgrade, oversized and expired envelopes', () => {
  ...
  assert.throws(
    () => parsePairingInput(JSON.stringify({ ...value, v: 1 })),
    /downgraded|Unsupported/,
  );
  assert.throws(
    () => parsePairingInput(JSON.stringify({ ...value, sessionRef: 'x' })),
    /session reference/,
  );
  assert.throws(
    () =>
      parsePairingInput(
        JSON.stringify({ ...value, expiresAt: new Date(Date.now() - 1).toISOString() }),
      ),
    /expired/,
  );
...
test('refresh rotation invalidates reuse and revocation denies access', () => {
  ...
  const grant = store.issue({
    peerKey: 'mobile',
    nodeId: 'node',
    capabilities: ['app.read' as PeerCapability],
  });
...
test('capabilities can only be intersected, never escalated', () => {
  assert.deepEqual(
    intersectCapabilities(
      ['app.read', 'app.write'] as PeerCapability[],
      ['app.read'] as PeerCapability[],
    ),
    ['app.read'],
  );
});
