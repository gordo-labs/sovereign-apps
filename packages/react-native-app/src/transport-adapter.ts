...
export type ReactNativeTransportState = 'idle' | 'starting' | 'ready' | 'stopping' | 'stopped' | 'failed';
...
type CandidateAddress = {
  target:
    ...
    | { kind: 'endpoint-address'; nodeId: string; directAddresses?: string[]; relayUrl?: string | null };
};
...
function normalizeAddress(raw: string): string {
  ...
  if (value.startsWith('iroh+direct://')) return normalizeAddress(value.slice('iroh+direct://'.length));
  if (value.startsWith('relay:')) return normalizeAddress(value.slice('relay:'.length));
  if (value.startsWith('iroh+relay://')) return normalizeAddress(value.slice('iroh+relay://'.length));
...
export function normalizeCandidate(candidate: TransportCandidate): CandidateAddress & { candidate: TransportCandidate } {
  if (!candidate.id.trim()) throw new ReactNativeTransportError('INVALID_DIAL_TARGET', 'Candidate id is empty');
  ...
  if (address.startsWith('{')) {
    let parsed: unknown;
    try { parsed = JSON.parse(address); } catch (error) {
      throw new ReactNativeTransportError('INVALID_TICKET', 'Candidate JSON ticket is malformed', { cause: error });
    }
    if (!parsed || typeof parsed !== 'object') throw new ReactNativeTransportError('INVALID_TICKET', 'Candidate JSON ticket is not an object');
    const value = parsed as { id?: unknown; addrs?: unknown; relay?: unknown };
    if (typeof value.id !== 'string' || value.id !== candidate.id) {
      throw new ReactNativeTransportError('INVALID_DIAL_TARGET', 'Ticket endpoint id does not match candidate id');
    }
    if (!Array.isArray(value.addrs) || value.addrs.length === 0 || value.addrs.some((item) => typeof item !== 'string')) {
    ...
    return {
      ...
      target: {
        ...
        relayUrl: addresses.find(isRelay) ?? (typeof value.relay === 'string' ? normalizeAddress(value.relay) : null),
      },
    };
  }
  return {
...
function raceAbort<T>(promise: Promise<T>, signal: AbortSignal | undefined, timeoutMs: number): Promise<T> {
  if (signal?.aborted) return Promise.reject(new ReactNativeTransportError('CANCELLED', 'Operation cancelled'));
  return new Promise<T>((resolve, reject) => {
    let timer: ReturnType<typeof setTimeout> | undefined;
    const abort = () => { if (timer) clearTimeout(timer); reject(new ReactNativeTransportError('CANCELLED', 'Operation cancelled')); };
    signal?.addEventListener('abort', abort, { once: true });
    timer = setTimeout(() => reject(new ReactNativeTransportError('TIMEOUT', `Operation exceeded ${timeoutMs}ms`)), timeoutMs);
    promise.then(resolve, (error) => reject(error)).finally(() => {
...
class NativeFramedStream implements FramedStream {
  ...
  constructor(private readonly connection: IrohBridgeConnection) {
    this.unsubscribe.push(connection.onMessage((data) => {
      const frame = data instanceof Uint8Array ? data : Uint8Array.from(data);
      if (frame.byteLength === 0 || frame.byteLength > MAX_FRAME_BYTES) return this.closeSilently();
  ...
  async read(options: { signal?: AbortSignal } = {}): Promise<Uint8Array | null> {
    ...
    return raceAbort(new Promise<Uint8Array | null>((resolve) => this.waiters.push(resolve)), options.signal, 30_000);
  }
  ...
  async write(frame: Uint8Array, options: { signal?: AbortSignal } = {}): Promise<void> {
    ...
    if (frame.byteLength === 0 || frame.byteLength > MAX_FRAME_BYTES) throw new ReactNativeTransportError('INVALID_FRAME', 'Frame must be 1..2MiB');
...
class NativeTransportSession implements TransportSession {
  ...
  private closed = false;
  constructor(readonly candidate: TransportCandidate, private readonly session: IrohBridgeSession) {}
...
export class ReactNativeIrohEndpoint implements TransportEndpoint {
  ...
  async start(context: { signal: AbortSignal }): Promise<void> {
    ...
    if (this.state === 'starting') throw new ReactNativeTransportError('STARTING', 'Endpoint is already starting');
    this.state = 'starting';
    try {
      ...
      this.state = 'failed';
      throw new ReactNativeTransportError('NATIVE_UNAVAILABLE', 'React Native Iroh native module failed to start', { cause: error });
    }
  }
  async stop(): Promise<void> {
    if (this.state === 'stopped' || this.state === 'idle') { this.state = 'stopped'; return; }
    this.state = 'stopping';
    try { await this.bridge.stop(); this.state = 'stopped'; }
    catch (error) { this.state = 'failed'; throw new ReactNativeTransportError('STOP_FAILED', 'Failed to stop native Iroh endpoint', { cause: error }); }
  }
  async candidates(): Promise<readonly TransportCandidate[]> { return this.configured; }
  async accept(): Promise<TransportSession> { throw new ReactNativeTransportError('OUTBOUND_ONLY', 'React Native endpoint does not accept incoming sessions'); }
  async connect(candidate: TransportCandidate, options: { signal?: AbortSignal; timeoutMs?: number } = {}): Promise<TransportSession> {
    if (this.state !== 'ready') throw new ReactNativeTransportError('NOT_READY', 'Start the endpoint before connecting');
    const normalized = normalizeCandidate(candidate);
    const call = this.bridge.openTargetSession({ target: normalized.target, alpn: DEFAULT_ALPN, timeoutMs: options.timeoutMs ?? 4_500 });
    try {
  ...
  async diagnostics(): Promise<{ packageVersion: string; nodeId: string; running: boolean }> { return bridgeDiagnostics(this.bridge); }
}
