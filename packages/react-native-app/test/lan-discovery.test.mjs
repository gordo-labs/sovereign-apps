...
const service = {
  txt: { v: '1', app: 'demo', node: 'node-a', addrs: '["192.168.1.2:42424"]', name: 'Demo' },
};

test('RN browser requests permission, deduplicates and emits candidate', async () => {
  const browser = fakeBrowser();
  const discovery = new ReactNativeLanDiscovery({
    browser,
...
test('RN browser exposes denied permission as explicit failure', async () => {
  ...
  await assert.rejects(
    discovery.start({ signal: new AbortController().signal, platform: 'android', now: Date.now }),
    /LAN_PERMISSION_DENIED/,
  );
});
