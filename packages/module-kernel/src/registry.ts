...
export interface Composition {
  readonly modules: readonly SovereignModule[];
  readonly optionalFailures: readonly string[];
}
export interface CompositionResult {
  readonly composition: Composition;
  readonly errors: ModuleCompositionError[];
}
...
export class ModuleRegistry {
  ...
  register(module: SovereignModule): void {
    if (this.modules.has(module.manifest.id))
      throw new ModuleCompositionError(`Duplicate module id: ${module.manifest.id}`);
    this.modules.set(module.manifest.id, module);
  }
  get(id: ModuleId): SovereignModule | undefined {
  ...
  validate(
    ...
    for (const [index, module] of chosen.entries()) {
      ...
      if (!module) {
        errors.push(new ModuleCompositionError(`Unknown module: ${id}`, { moduleId: id }));
        continue;
      }
      const manifest = module.manifest;
      if (manifest.platforms && !manifest.platforms.includes(platform))
        errors.push(
          new ModuleCompositionError(`Module ${id} does not support platform ${platform}`, {
            moduleId: id,
            platform,
          }),
        );
      if (!module.availability.available && !manifest.optional)
        errors.push(new ModuleUnavailableError(id, module.availability.reason));
      for (const dependency of manifest.dependencies ?? [])
        if (!selected.includes(dependency))
          errors.push(
            new ModuleCompositionError(`Module ${id} requires ${dependency}`, {
              moduleId: id,
              dependency,
            }),
          );
      for (const conflict of manifest.conflicts ?? [])
        if (selected.includes(conflict))
          errors.push(
            new ModuleCompositionError(`Module ${id} conflicts with ${conflict}`, {
              moduleId: id,
              conflict,
            }),
          );
      for (const field of manifest.configSchema ?? [])
        if (field.required && config[field.name] === undefined)
          errors.push(
            new ModuleCompositionError(`Module ${id} requires config.${field.name}`, {
              moduleId: id,
              field: field.name,
            }),
          );
    }
    const optionalFailures = chosen
      .filter(
        (m): m is SovereignModule =>
          !!m &&
          (!m.availability.available ||
            Boolean(m.manifest.platforms && !m.manifest.platforms.includes(platform))),
      )
      .filter((m) => m.manifest.optional)
      .map((m) => m.manifest.id);
    return {
      composition: {
        modules: chosen.filter(
          (m): m is SovereignModule => !!m && !optionalFailures.includes(m.manifest.id),
        ),
        optionalFailures,
      },
      errors,
  ...
  async start(
    ...
    if (result.errors.length)
      throw new ModuleCompositionError(result.errors.map((e) => e.message).join('; '), {
        errors: result.errors.map((e) => e.details),
      });
  ...
  private order(modules: readonly SovereignModule[]): SovereignModule[] {
    ...
    const visit = (module: SovereignModule): void => {
      ...
      if (visiting.has(module.manifest.id))
        throw new ModuleCompositionError(`Dependency cycle at ${module.manifest.id}`);
      visiting.add(module.manifest.id);
      for (const dependency of module.manifest.dependencies ?? []) {
        const dependencyModule = selected.get(dependency);
        if (dependencyModule) visit(dependencyModule);
      }
  ...
  async stop(composition: Composition): Promise<void> {
    for (const module of [...composition.modules].reverse()) await module.stop();
  }
}
