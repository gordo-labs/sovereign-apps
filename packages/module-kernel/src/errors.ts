...
export class ModuleCompositionError extends ModuleKernelError {
  constructor(message: string, details?: Readonly<Record<string, unknown>>) {
    super('invalid_composition', message, details);
    this.name = 'ModuleCompositionError';
  }
}
export class ModuleUnavailableError extends ModuleKernelError {
  constructor(moduleId: string, reason?: string) {
    super('unavailable', `Module ${moduleId} is unavailable${reason ? `: ${reason}` : ''}`, {
      moduleId,
    });
    this.name = 'ModuleUnavailableError';
  }
}
export class OperationCancelledError extends ModuleKernelError {
  constructor(operation: string) {
    super('cancelled', `${operation} was cancelled`);
    this.name = 'OperationCancelledError';
  }
}
export class OperationTimeoutError extends ModuleKernelError {
...
export async function withTimeout<T>(
  ...
  try {
    ...
    if (parent?.aborted || (controller.signal.aborted && !(error instanceof OperationTimeoutError)))
