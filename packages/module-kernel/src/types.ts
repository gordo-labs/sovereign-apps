...
export interface Discovery extends SovereignModule {
  discover(options?: {
    signal?: AbortSignal;
    timeoutMs?: number;
...
export interface BootstrapEnvelope {
  readonly version: 1;
  readonly kind: 'qr' | 'ble' | 'nfc' | 'deep-link' | 'file';
  readonly peerId: string;
  readonly expiresAt: string;
  readonly material: Readonly<Record<string, string>>;
  readonly signature?: string;
}
export interface Bootstrap extends SovereignModule {
  export(envelope: BootstrapEnvelope, signal?: AbortSignal): Promise<Uint8Array>;
  import(data: Uint8Array, signal?: AbortSignal): Promise<BootstrapEnvelope>;
}
...
export interface TransportEndpoint extends SovereignModule {
  candidates(signal?: AbortSignal): Promise<readonly TransportCandidate[]>;
  accept(signal?: AbortSignal): Promise<TransportSession>;
  connect(
...
export interface TransportSession {
  ...
  openStream(options?: { signal?: AbortSignal }): Promise<FramedStream>;
  close(reason?: string): Promise<void>;
}
export interface FramedStream {
  readonly id: string;
  read(options?: { signal?: AbortSignal }): Promise<Uint8Array | null>;
  write(frame: Uint8Array, options?: { signal?: AbortSignal }): Promise<void>;
  close(): Promise<void>;
}
...
export interface PairingPolicy {
  verifyBootstrap(
    ...
    signal?: AbortSignal,
  ): Promise<{ peerId: string; accepted: boolean }>;
  authenticate(
    session: TransportSession,
    requested: readonly string[],
    signal?: AbortSignal,
  ): Promise<AuthResult>;
  revoke(peerId: string, signal?: AbortSignal): Promise<void>;
}
...
export interface RoutePolicy {
  rank(
    ...
    signal?: AbortSignal,
  ): Promise<readonly TransportCandidate[]>;
  observe(event: RouteEvent): void;
}
export interface RouteEvent {
  readonly candidateId: string;
  readonly state: 'available' | 'connecting' | 'connected' | 'failed' | 'closed';
  readonly at: string;
  readonly errorCode?: string;
}
export interface AppCodec<T = unknown> {
  readonly capabilities: readonly string[];
  encode(value: T, signal?: AbortSignal): Uint8Array;
  decode(frame: Uint8Array, signal?: AbortSignal): T;
