...
export function encodeBootstrapArtifact(artifact: BootstrapArtifact): Uint8Array {
  ...
  if (bytes.byteLength > MAX_OFFLINE_ARTIFACT_BYTES)
    throw new Error('Bootstrap artifact exceeds size limit');
  return bytes;
}
...
export function validateBootstrapArtifact(
  ...
  if (bytes.byteLength === 0 || bytes.byteLength > maxBytes)
    throw new Error('Bootstrap artifact is empty or oversized');
  let text: string;
  try {
    text = new TextDecoder('utf-8', { fatal: true }).decode(bytes);
  } catch {
    throw new Error('Bootstrap artifact is not valid UTF-8');
  }
  const envelope = parsePairingInput(text, { allowManual: true });
  if (Date.parse(envelope.expiresAt) <= (options.now ?? Date.now()))
    throw new Error('Bootstrap artifact expired');
  return Object.freeze({
    version: OFFLINE_ARTIFACT_VERSION,
    kind: 'sovereign-pairing-bootstrap',
    envelope,
...
export class BootstrapReplayStore {
  ...
  consume(artifact: BootstrapArtifact, now = Date.now()): void {
    ...
    if (Date.parse(artifact.envelope.expiresAt) <= now)
      throw new Error('Bootstrap artifact expired');
  ...
  has(sessionRef: string): boolean {
    return this.consumed.has(sessionRef);
  }
  clear(): void {
    this.consumed.clear();
  }
...
export function redactBootstrapText(): string {
  return '[sovereign pairing artifact redacted]';
}
