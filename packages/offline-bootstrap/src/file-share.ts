...
export type BootstrapFile = Readonly<{
  filename: string;
  mime: typeof import('./artifact.js').PAIRING_FILE_MIME;
  bytes: Uint8Array;
}>;
export function exportBootstrapFile(
  envelope: SecurePairingQrEnvelope,
  filename = `pairing${'.sovereign-pairing'}`,
): BootstrapFile {
  const safe = basename(filename);
  if (safe !== filename || extname(safe) !== '.sovereign-pairing')
    throw new Error('Invalid pairing filename');
  return {
    filename: safe,
    mime: 'application/vnd.sovereign-apps.pairing+json',
    bytes: encodeBootstrapArtifact(createBootstrapArtifact(envelope)),
  };
}

export function importBootstrapFile(
  bytes: Uint8Array,
  metadata: { filename: string; mime: string },
  options: { now?: number } = {},
): BootstrapArtifact {
  if (
    basename(metadata.filename) !== metadata.filename ||
    !metadata.filename.endsWith('.sovereign-pairing')
  )
    throw new Error('Invalid pairing filename');
  if (metadata.mime !== 'application/vnd.sovereign-apps.pairing+json')
    throw new Error('Pairing file MIME mismatch');
...
export async function readBootstrapFile(
  ...
  if (basename(filename) !== filename || !filename.endsWith('.sovereign-pairing'))
    throw new Error('Path traversal or invalid pairing filename');
  const handle = await open(path, 'r');
  try {
    const stat = await handle.stat();
    const maxBytes = options.maxBytes ?? 2048;
    if (!stat.isFile() || stat.size > maxBytes)
      throw new Error('Pairing file is oversized or not a regular file');
...
export async function handoffBootstrapFile(
  path: string,
  metadata: { filename?: string; mime: string },
