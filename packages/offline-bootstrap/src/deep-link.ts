...
export type DeepLinkConfig = Readonly<{
  schemes: readonly string[];
  hosts?: readonly string[];
  paths?: readonly string[];
}>;
export const DEFAULT_DEEP_LINK_CONFIG: DeepLinkConfig = Object.freeze({
  schemes: ['sovereign'],
  hosts: ['pair'],
  paths: ['/pair', '/'],
});
...
function checkedConfig(config: DeepLinkConfig): DeepLinkConfig {
  if (
    ...
    config.schemes.some((scheme) => !/^[a-z][a-z0-9+.-]*$/i.test(scheme))
  )
    throw new Error('Invalid deep-link scheme allowlist');
  return config;
}
...
export function parseDeepLink(
  ...
  if (!config.schemes.includes(url.protocol.slice(0, -1)))
    throw new Error('Deep-link scheme is not allowed');
  if (url.username || url.password || url.port || url.hash)
    throw new Error('Deep link contains forbidden URL components');
  if (config.hosts && !config.hosts.includes(url.hostname))
    throw new Error('Deep-link host is not allowed');
  if (config.paths && !config.paths.includes(url.pathname))
    throw new Error('Deep-link path is not allowed');
  if ([...url.searchParams.keys()].some((key) => key !== 'data') || !url.searchParams.get('data'))
    throw new Error('Deep link must contain only data');
  let bytes: Uint8Array;
  try {
    bytes = new Uint8Array(Buffer.from(url.searchParams.get('data')!, 'base64url'));
  } catch {
    throw new Error('Malformed deep-link data');
  }
  return validateBootstrapArtifact(bytes, options);
}

export async function handoffDeepLink(
  value: string,
