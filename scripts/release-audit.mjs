...
const forbidden =
  /(^|\/)(node_modules|src|test|tests|\.git|\.env)(\/|$)|(^|\/)(pnpm-lock\.yaml|.*\.pem|.*\.key|.*\.p12|.*\.tgz)$/i;
...
async function main() {
  ...
  for (const name of publishable) {
    ...
    if (!pkg.publishConfig?.provenance)
      errors.push(`${name}: publishConfig.provenance must be true`);
  ...
  const sbom = {
    bomFormat: 'CycloneDX',
    specVersion: '1.5',
    serialNumber: `urn:uuid:sovereign-apps-${Date.now()}`,
    metadata: {
      timestamp: report.generatedAt,
      tools: [{ vendor: 'Gordo Labs', name: 'sovereign-apps release-audit', version: '0.0.1' }],
    },
    components: [
      ...new Map(
  ...
  if (errors.length) {
    ...
    console.log(
      `${packMode ? 'Packed' : 'Audited'} ${report.packages.length} release candidates; no forbidden files found.`,
    );
  }
}
