...
const now = Date.now();
const evidence = files(evidenceDir)
  .map(readJson)
  .filter((item) => item.schemaVersion === '1.0');
const required = [];
...
for (const item of required) {
  const matches = evidence
    .filter((entry) => entry.platformId === item.platformId && entry.caseId === item.caseId)
  ...
  if (now - Date.parse(latest.generatedAt) > maxAgeHours * 3600000) {
    stale.push({ ...item, evidenceId: latest.evidenceId });
    continue;
  }
...
const issues = [
  ...missing.map((item) => ({ type: 'missing', ...item })),
  ...stale.map((item) => ({ type: 'stale', ...item })),
  ...failed.map((item) => ({ type: 'failed', ...item })),
];
if (unsigned) issues.push({ type: 'unsigned' });
const manifest = {
  schemaVersion: '1.0',
  channel: options.channel ?? 'ci',
  generatedAt: new Date().toISOString(),
  commit: gitCommit(),
  supportMatrixVersion: matrix.schemaVersion,
  requiredEvidenceWindowHours: maxAgeHours,
  packages: packageVersions(root),
  summary: {
    status: issues.length ? 'red' : 'green',
    stableEligible: issues.length === 0,
    missing,
    stale,
    failed,
    unsigned,
...
if (options['require-signature'] && unsigned) {
  manifest.summary.status = 'red';
  manifest.summary.stableEligible = false;
}
