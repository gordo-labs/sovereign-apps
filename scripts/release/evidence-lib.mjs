...
const SENSITIVE =
  /(qr|token|secret|private|bearer|password|credential|endpoint|device.?id|node.?id|key)/i;
...
export function sanitizeText(value, limit = 2000) {
  ...
  if (SENSITIVE.test(text))
    throw new Error(
      'Sensitive field detected; do not put keys, tokens, QR data or endpoints in evidence.',
    );
...
export function packageVersions(root) {
  ...
  for (const path of [
    ...
    'packages/web-presence/package.json',
  ]) {
    try {
...
export function toolchainVersions() {
  ...
  for (const [name, [command, args]] of Object.entries(commands)) {
    try {
      versions[name] = execFileSync(command, args, {
        encoding: 'utf8',
...
export function writeEvidence({
  ...
  if (!['pass', 'fail', 'skipped', 'blocked'].includes(result))
    throw new Error(`Unsupported result: ${result}`);
  const safeLogs = logs.map((path) => {
    if (SENSITIVE.test(basename(path)))
      throw new Error(`Sensitive log filename: ${basename(path)}`);
    return {
      name: basename(path)
  ...
  const evidence = {
    schemaVersion: '1.0',
    evidenceId: `${Date.now().toString(36)}-${randomUUID().slice(0, 8)}`,
    platformId,
    caseId,
    commit: gitCommit(),
